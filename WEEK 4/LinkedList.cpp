...
bool removeHead(List *&L)
{
    ...
    if (L->p_head != nullptr && L->p_head == L->p_tail) {
        NODE* p = L->p_head;
        L->p_head = nullptr;
        L->p_tail = nullptr;
        delete p;
...
void removeBefore(List *&L, int val)
{
    if (!L || !L->p_head || !L->p_head->key) return;
    NODE* prev = nullptr;
    NODE* cur = L->p_head;
    NODE* next = cur->p_next;

    while (next && next->key != val) {
        prev = cur;
        cur = next;
        next = next->p_next;
    }
    if (next && cur == L->p_head) {
        removeHead(L);
    }
    else if (next) {
        prev->p_next = next;
...
bool addBefore(List *&L, int data, int val)
{
    ...
    NODE* prev = nullptr, * cur = L->p_head;
...
List *reverseList(List *L)
{
    ...
    result->p_head = result->p_tail = nullptr;

    NODE* cur = L->p_head;
    while (cur) {
        NODE* newNode = new NODE;
        newNode->key = cur->key;
        newNode->p_next = result->p_head;
        result->p_head = newNode;

        if (!result->p_tail) {
            result->p_tail = newNode;
        }
        cur = cur->p_next;
    }
    return result;
    ...
    while (cur) {
        ...
        while (run->p_next) {
            if (run->p_next->key == cur->key) {
                NODE* temp = run->p_next;
                run->p_next = temp->p_next;
