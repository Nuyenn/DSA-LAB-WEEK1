...
void removeBefore(d_List*& L, int val) {
    ...
    while (cur) {
        if (cur->key == val) {
            d_NODE* target = cur->pPrev;
            if (target == L->pHead) {
                removeHead(L);
            }
            else {
                target->pPrev->pNext = cur;
                cur->pPrev = target->pPrev;
                delete target;
            }
            return;
        }
...
void removeAfter(d_List*& L, int val) {
    ...
    while (cur) {
        if (cur->key == val && cur->pNext) {
            d_NODE* target = cur->pNext;
            cur->pNext = target->pNext;
            if (target->pNext) {
                target->pNext->pPrev = cur;
            }
            else {
                L->pTail = cur;
            }
...
bool addBefore(d_List*& L, int data, int val) {
    ...
    if (cur == L->pHead) return addHead(L, data);
...
bool removeElement(d_List*& L, int key) {
    ...
    while (cur) {
        if (cur->key == key) {
            ...
            if (temp == L->pHead) L->pHead = temp->pNext;
            if (temp == L->pTail) L->pTail = temp->pPrev;
            if (temp->pPrev) temp->pPrev->pNext = temp->pNext;
            if (temp->pNext) temp->pNext->pPrev = temp->pPrev;
